import { SafeAreaView, ScrollView, Text, StyleSheet, View, ActivityIndicator, TouchableOpacity, Modal, TextInput, Alert, RefreshControl, KeyboardAvoidingView, Platform } from 'react-native';
import { useTwinStore } from '../store/useTwinStore';
import { supabase } from '../lib/supabase';
import { useState, useEffect, useCallback, useRef } from 'react';
import { HeartPulse, Sparkles, Plus, X, TrendingUp, TrendingDown, Minus, Smile, Frown, Angry, Heart, Zap, Shield, Star } from 'lucide-react-native';
...
export default function Mood() {
  ...
  const fetchMoods = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      const { data, error: fetchError } = await supabase.from('emotional_timeline').select('*').eq('user_id', userId).gte('created_at', sevenDaysAgo).order('created_at', { ascending: false });
      ...
      if (entries.length > 0) {
        ...
        const freq: Record<string, number> = {}; emotions.forEach(e => { freq[e] = (freq[e] || 0) + 1; });
    ...
    } catch (e) { if (!cancelledRef.current) setError(t('فشل تحميل البيانات', 'Failed to load data')); }
  ...
  const handleAddMood = async () => {
    ...
    try {
      await supabase.from('emotional_timeline').insert({ user_id: userId, primary_emotion: selectedMood, intensity: 0.7, valence: selectedMood === 'joy' || selectedMood === 'love' ? 0.8 : selectedMood === 'sadness' || selectedMood === 'anger' ? -0.5 : 0.2 });
      setShowAddModal(false); setSelectedMood('joy'); setMoodNote(''); fetchMoods(true);
    } catch (e) { Alert.alert(t('خطأ', 'Error'), t('فشل تسجيل المشاعر', 'Failed to log mood')); }
    finally { setSaving(false); }
  };

  const bg = isDark ? '#1A1A1A' : '#F8F6F2'; const card = isDark ? '#2A2A2A' : '#FFF'; const border = isDark ? '#444' : '#F0F0F0'; const txt = isDark ? '#FFF' : '#1A1A1A'; const sub = isDark ? '#888' : '#666';
  if (loading) return <SafeAreaView style={[s.safe, { backgroundColor: bg }]}><ActivityIndicator size="large" color="#6B21A8" style={s.loader} /></SafeAreaView>;

  return (
    <SafeAreaView style={[s.safe, { backgroundColor: bg }]}>
      <ScrollView contentContainerStyle={[s.container, { backgroundColor: bg }]} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={['#6B21A8']} />}>
        ...
        {summary && (
          <View style={[s.summaryCard, { backgroundColor: card, borderColor: border }]}>
            <View style={[s.summaryHeader, isAr && { flexDirection: 'row-reverse' }]}><Sparkles size={20} stroke="#A855F7" /><Text style={[s.summaryTitle, { color: txt }]}>{t('تحليل الأسبوع', 'Weekly Analysis')}</Text></View>
            <View style={s.statsRow}>
              <View style={s.stat}><Text style={[s.statValue, { color: '#A855F7' }]}>{Math.round(summary.avgValence * 100)}%</Text><Text style={[s.statLabel, { color: sub }]}>{t('إيجابية', 'Positivity')}</Text></View>
              <View style={s.stat}><Text style={[s.statValue, { color: '#F59E0B' }]}>{Math.round(summary.avgIntensity * 100)}%</Text><Text style={[s.statLabel, { color: sub }]}>{t('شدة', 'Intensity')}</Text></View>
              <View style={s.stat}>{summary.trend === 'up' ? <TrendingUp size={24} stroke="#10B981" /> : summary.trend === 'down' ? <TrendingDown size={24} stroke="#EF4444" /> : <Minus size={24} stroke={sub} />}<Text style={[s.statLabel, { color: sub }]}>{t('الاتجاه', 'Trend')}</Text></View>
            </View>
          </View>
        )}

        <View style={[s.relationshipCard, { backgroundColor: card, borderColor: border }]}>
          <View style={[s.summaryHeader, isAr && { flexDirection: 'row-reverse' }]}><Heart size={20} stroke="#EC4899" /><Text style={[s.summaryTitle, { color: txt }]}>{t('مشاعر العلاقة', 'Relationship Mood')}</Text></View>
          <View style={s.relGrid}>
            {[{ label: t('ثقة', 'Trust'), value: relationshipDims.trust, color: '#3B82F6', icon: Shield }, { label: t('ارتباط', 'Attachment'), value: relationshipDims.affection, color: '#EC4899', icon: Heart }, { label: t('راحة', 'Comfort'), value: relationshipDims.empathy, color: '#10B981', icon: Star }].map((item, i) => (
              <View key={i} style={s.relItem}><item.icon size={16} stroke={item.color} /><View style={[s.relBar, { backgroundColor: isDark ? '#444' : '#F0F0F0' }]}><View style={[s.relFill, { width: `${Math.min(item.value, 100)}%`, backgroundColor: item.color }]} /></View><Text style={[s.relLabel, { color: sub }]}>{item.label}</Text></View>
            ))}
          </View>
        </View>

        <Text style={[s.sectionTitle, { color: txt }]}>{t('آخر المشاعر', 'Recent Moods')}</Text>
        {recentMoods.length === 0 ? <Text style={[s.empty, { color: sub }]}>{t('لا توجد بيانات بعد', 'No data yet')}</Text> : recentMoods.slice(0, 7).map((entry, i) => {
      ...
      <Modal visible={showAddModal} transparent animationType="fade" onRequestClose={() => setShowAddModal(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.modalOverlay}>
          <View style={[s.modalContent, isDark && { backgroundColor: '#2A2A2A' }]}>
            ...
            <View style={s.moodGrid}>{MOOD_OPTIONS.map((mood) => (<TouchableOpacity key={mood.value} style={[s.moodOption, { borderColor: selectedMood === mood.value ? mood.color : border, backgroundColor: selectedMood === mood.value ? mood.color + '20' : 'transparent' }]} onPress={() => setSelectedMood(mood.value)}><Text style={s.moodOptionEmoji}>{mood.emoji}</Text><Text style={[s.moodOptionLabel, { color: selectedMood === mood.value ? mood.color : sub }]}>{isAr ? mood.label_ar : mood.label_en}</Text></TouchableOpacity>))}</View>
            <TextInput style={[s.input, { backgroundColor: isDark ? '#333' : '#F8F6F2', color: txt, borderColor: isDark ? '#444' : '#E0D9F5' }]} placeholder={t('ملاحظة (اختياري)', 'Note (optional)')} placeholderTextColor={sub} value={moodNote} onChangeText={setMoodNote} maxLength={200} />
            <TouchableOpacity style={[s.saveBtn, { opacity: saving ? 0.6 : 1 }]} onPress={handleAddMood} disabled={saving}>{saving ? <ActivityIndicator color="#FFF" /> : <Text style={s.saveBtnText}>{t('تسجيل', 'Log Mood')}</Text>}</TouchableOpacity>
          </View>
...
const s = StyleSheet.create({
  safe: { flex: 1 }, container: { padding: 20, paddingBottom: 40 }, loader: { flex: 1, marginTop: 80 },
  ...
  stat: { alignItems: 'center' }, statValue: { fontSize: 24, fontWeight: '800' }, statLabel: { fontSize: 12, marginTop: 4 },
